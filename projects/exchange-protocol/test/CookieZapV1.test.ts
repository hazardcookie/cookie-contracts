...
const CookieFactory = artifacts.require("./CookieFactory.sol");
const CookiePair = artifacts.require("./CookiePair.sol");
const CookieRouter = artifacts.require("./CookieRouter.sol");
const CookieZapV1 = artifacts.require("./CookieZapV1.sol");
const WBNB = artifacts.require("./WBNB.sol");

contract("CookieZapV1", ([alice, bob, carol, david, erin]) => {
  ...
  before(async () => {
    // Deploy Factory
    cookieFactory = await CookieFactory.new(alice, { from: alice });

    // Deploy Wrapped BNB
    wrappedBNB = await WBNB.new({ from: alice });

    // Deploy Router
    cookieRouter = await CookieRouter.new(cookieFactory.address, wrappedBNB.address, { from: alice });

    // Deploy ZapV1
    maxZapReverseRatio = 100; // 1%
    cookieZap = await CookieZapV1.new(wrappedBNB.address, cookieRouter.address, maxZapReverseRatio, { from: alice });
    ...
    // Create 3 LP tokens
    let result = await cookieFactory.createPair(tokenA.address, wrappedBNB.address, { from: alice });
    pairAB = await CookiePair.at(result.logs[0].args[2]);

    result = await cookieFactory.createPair(wrappedBNB.address, tokenC.address, { from: alice });
    pairBC = await CookiePair.at(result.logs[0].args[2]);

    result = await cookieFactory.createPair(tokenA.address, tokenC.address, { from: alice });
    pairAC = await CookiePair.at(result.logs[0].args[2]);
    ...
    for (let thisUser of [alice, bob, carol, david, erin]) {
      ...
      await tokenA.approve(cookieRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenA.approve(cookieZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenC.approve(cookieRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await tokenC.approve(cookieZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await wrappedBNB.approve(cookieRouter.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await wrappedBNB.approve(cookieZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairAB.approve(cookieZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairBC.approve(cookieZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });

      await pairAC.approve(cookieZap.address, constants.MAX_UINT256, {
        from: thisUser,
      });
    }
  });
  ...
  describe("Normal cases for liquidity provision and zap ins", async () => {
    it("User adds liquidity to LP tokens", async function () {
      ...
      // 1 A = 1 C
      let result = await cookieRouter.addLiquidity(
        tokenC.address,
        tokenA.address,
      ...
      // 1 BNB = 100 A
      result = await cookieRouter.addLiquidityETH(
        tokenA.address,
    ...
    it("User completes zapIn with tokenA (pair tokenA/tokenC)", async function () {
      ...
      const estimation = await cookieZap.estimateZapInSwap(tokenToZap, parseEther("1"), lpToken);
      ...
      const result = await cookieZap.zapInToken(tokenToZap, tokenAmountIn, lpToken, minTokenAmountOut, {
        from: carol,
      });
      ...
      console.info("Balance tokenA: " + formatUnits(String(await tokenA.balanceOf(cookieZap.address)), 18));
      console.info("Balance WBNB: " + formatUnits(String(await wrappedBNB.balanceOf(cookieZap.address)), 18));
      console.info("Balance tokenC: " + formatUnits(String(await tokenC.balanceOf(cookieZap.address)), 18));
    });
    ...
    it("User completes zapIn with BNB (pair BNB/tokenC)", async function () {
      ...
      const estimation = await cookieZap.estimateZapInSwap(wrappedBNB.address, parseEther("1"), lpToken);
      assert.equal(estimation[2], tokenC.address);

      // Setting up slippage at 0.5%
      const minTokenAmountOut = new BN(estimation[1].toString()).mul(new BN("9995")).div(new BN("10000"));

      const result = await cookieZap.zapInBNB(lpToken, minTokenAmountOut, {
        from: carol,
    ...
    it("User completes zapInRebalancing with BNB (pair BNB/tokenC)", async function () {
      ...
      const estimation = await cookieZap.estimateZapInRebalancingSwap(
        wrappedBNB.address,
        tokenC.address,
        token0AmountIn,
        token1AmountIn,
      ...
      const result = await cookieZap.zapInBNBRebalancing(
        tokenC.address,
        token1AmountIn,
        lpToken,
        maxTokenAmountIn,
    ...
    it("User completes zapInRebalancing with tokens (tokenA/tokenC)", async function () {
      ...
      const result = await cookieZap.zapInTokenRebalancing(
        tokenA.address,
        tokenC.address,
        token0AmountIn,
        token1AmountIn,
    ...
    it("User completes zapOut to token (tokenA/tokenC)", async function () {
      ...
      const estimation = await cookieZap.estimateZapOutSwap(lpToken, lpTokenAmount, tokenToReceive);
      assert.equal(estimation[2], tokenC.address);

      const minTokenAmountOut = new BN(estimation[1].toString()).mul(new BN("9995")).div(new BN("10000"));

      const result = await cookieZap.zapOutToken(lpToken, tokenToReceive, lpTokenAmount, minTokenAmountOut, {
        from: carol,
      });
    ...
    it("User completes zapOut to BNB (BNB/tokenC)", async function () {
      ...
      const result = await cookieZap.zapOutBNB(lpToken, lpTokenAmount, minTokenAmountOut, {
        from: carol,
      });
    ...
    it("Zap estimation fail if wrong tokens", async function () {
      await expectRevert(
        cookieZap.estimateZapInSwap(wrappedBNB.address, parseEther("1"), pairAC.address),
        "Zap: Wrong tokens"
      );
      await expectRevert(
        cookieZap.estimateZapInRebalancingSwap(
          tokenA.address,
          wrappedBNB.address,
          parseEther("1"),
          parseEther("1"),
      ...
      await expectRevert(
        cookieZap.estimateZapOutSwap(pairAC.address, parseEther("1"), wrappedBNB.address),
        "Zap: Token not in LP"
      );
    });
    ...
    it("Zap estimations work as expected", async function () {
      ...
      const estimation0 = await cookieZap.estimateZapInRebalancingSwap(
        tokenA.address,
        tokenC.address,
        parseEther("0.5"),
        parseEther("1"),
      ...
      const estimation1 = await cookieZap.estimateZapInRebalancingSwap(
        tokenC.address,
        tokenA.address,
        parseEther("1"),
        parseEther("0.5"),
      ...
      const estimation2 = await cookieZap.estimateZapInSwap(tokenA.address, parseEther("5"), pairAC.address);
      const estimation3 = await cookieZap.estimateZapInRebalancingSwap(
        tokenA.address,
        tokenC.address,
        parseEther("5"),
        parseEther("0"),
    ...
    it("Cannot zap if wrong direction/tokens used", async function () {
      await expectRevert(
        cookieZap.zapInToken(tokenA.address, parseEther("1"), pairBC.address, parseEther("0.51"), { from: carol }),
        "Zap: Wrong tokens"
      );
      await expectRevert(
        cookieZap.zapInBNB(pairAC.address, parseEther("0.51"), { from: carol, value: parseEther("0.51").toString() }),
        "Zap: Wrong tokens"
      );

      await expectRevert(
        cookieZap.zapOutToken(pairBC.address, tokenA.address, parseEther("0.51"), parseEther("0.51"), { from: carol }),
        "Zap: Token not in LP"
      );

      await expectRevert(
        cookieZap.zapOutBNB(pairAC.address, parseEther("0.51"), parseEther("0.51"), { from: carol }),
        "Zap: Token not in LP"
      );

      await expectRevert(
        cookieZap.zapInTokenRebalancing(
          tokenA.address,
          tokenC.address,
          parseEther("1"),
          parseEther("1"),
